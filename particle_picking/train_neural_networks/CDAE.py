...
noise_folder = folder + 'noise_proj2/'
noNoise_folder = folder + 'noNoise_proj2/'

train, target = import_synth_data(noise_folder, noNoise_folder, 128, 0, 750000)
...
def create_model_dense(training_data, full_training, lr):
	...
	x = layers.MaxPooling2D(pool_size=(1,1), padding='same')(x)#5
	...
	x = layers.MaxPooling2D(pool_size=(1,1), padding='same')(x)#10
	...
	x = layers.UpSampling2D((1,1))(x)#33
	...
	x = layers.UpSampling2D((1,1))(x)#37
...
def train_model(train_data, train_target):
	...
	history = autoencoder.fit(x=train_data, y=train_target, epochs=100, batch_size=32, verbose=1, validation_data = (val_train[1:], val_target[1:]), callbacks=[es])
...
model_save_name = '../../bent_actin128/train_neural_network/750000training_CCC9749.h5'
...
fig,ax = plt.subplots(2,3); _=ax[0,0].imshow(train[check_num,:,:,0].astype('float32'), cmap=cm); _=ax[0,1].imshow(target[check_num,:,:,0].astype('float32'), cmap=cm); _=ax[1,0].imshow(predict_conv.astype('float32'), cmap=cm);_=ax[1,1].imshow(predict_dense,cmap=cm);  #plt.show(block=False)
...
with mrcfile.open(real_data_dir + 'beta_actin_Au_0010.mrcs') as mrc:
	real_data = mrc.data
...
for i in range(5, 6):
	check_num = i
	real_img = skimage.transform.resize(real_data[check_num], (128,128), anti_aliasing=True)
	real_img = (real_img - np.mean(real_img)) / np.std(real_img)
	real_img = np.expand_dims(np.expand_dims(real_img,axis=0),axis=-1).astype('float16')
	cm = plt.cm.gray
	predict_conv = -1.0*autoencoder_three.predict(1.0*real_img)[0,:,:,0]
	predict_dense = autoencoder_three.predict(1.0*real_img)[0,:,:,0]
	fig,ax = plt.subplots(2,3); _=ax[0,0].imshow(real_img[0,:,:,0].astype('float32'), cmap=cm); _=ax[0,1].imshow(real_img[0,:,:,0].astype('float32'), cmap=cm); _=ax[1,0].imshow(predict_conv.astype('float32'), cmap=cm);_=ax[1,1].imshow(predict_dense.astype('float32'),cmap=cm);  #plt.show(block=False)
	...
	if(i == 5):
		print(i)
		with mrcfile.new(real_data_dir + 'pred_bent%02d.mrc'%i, overwrite=True) as mrc:
			mrc.set_data(predict_dense.astype('float32'))
		
		with mrcfile.new(real_data_dir + 'noisy_bent%02d.mrc'%i, overwrite=True) as mrc:
			mrc.set_data(real_data[check_num].astype('float32'))
...
def plot_history(history):
	...
	plt.title('Loss'); plt.ylim(ymin=-1,ymax=0); plt.legend((p1,p2), ('Training Loss', 'Validation Loss'), loc='upper right', shadow=True)
	plt.show()
