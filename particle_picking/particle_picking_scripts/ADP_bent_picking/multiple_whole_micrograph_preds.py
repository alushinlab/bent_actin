...
file_names = sorted(glob.glob('../Micrographs_bin4/*.mrc'))[1161:] #You may specify range here
for i in tqdm(range(0, len(file_names))):
	...
	for j in range(0, len(filaments)):
		...
		curvature, r = spline_curv_estimate(filaments[j], len(filaments[j]), 10, 3, 50) # sampling, buff, min_threshold
		if(curvature != []):
			r_curv_range = r[np.argwhere(np.logical_and(10<curvature, curvature<900))][:,0,:]
			kept_curvs = curvature[np.argwhere(np.logical_and(10<curvature, curvature<900))]
	...
	extracted_filaments_oneFilPerParticle = []
	for j in range(0, len(extractions_full)):
		extracted_filaments_oneFilPerParticle.append(isolate_one_filament(extractions_full[j], FCN))
	
	extractions_full = extracted_filaments_oneFilPerParticle
	
	curv_preds = []
	for j in range(0, len(rs_full)):
		measured_curv = curv_measurement.predict(np.expand_dims(extractions_full[j], axis=-1))
		rs_full[j][:,3] = measured_curv[:,1]
		rs_full[j][:,4] = measured_curv[:,0]
		curv_preds.append(measured_curv)
	
	################################################################################
	# Prepare star file
	header = '# RELION; version 3.0\n\ndata_\n\nloop_ \n_rlnCoordinateX #1 \n_rlnCoordinateY #2 \n_rlnClassNumber #3 \n_rlnAutopickFigureOfMerit #4 \n_rlnHelicalTubeID #5 \n_rlnAngleTiltPrior #6 \n_rlnAnglePsiPrior #7 \n_rlnHelicalTrackLength #8 \n_rlnAnglePsiFlipRatio #9 \n_rlnSplineCurvMeasure #10 \n'
	star_file = header
	for j in range(0, len(rs_full)):
		for k in range(0, len(rs_full[j])):
			if(rs_full[j][k][4] <475):
				star_file = star_file + starify(rs_full[j][k][1]*4.0,rs_full[j][k][0]*4.0,1, 1.0, j+1, 90.0, rs_full[j][k][3], 0.0, 0.5, rs_full[j][k][2])
