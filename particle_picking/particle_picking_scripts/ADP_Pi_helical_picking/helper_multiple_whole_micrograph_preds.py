...
def skeletonize_and_prune_nubs(stitch_back, intersect_removal_box_size, prune_length):
	...
	skel = skeletonize_3d(sem_seg_img.astype('float32')>0.9)
	skel[skel>0] = 1
	...
	skel_pruned = skeletonize_3d(skel_pruned.astype('float32')>0.85)
...
def spline_curv_estimate(filament, smooth_param, sampling, buff, min_threshold):
	...
	psis = r_prime / np.linalg.norm(r_prime, axis=0)
	psis = np.degrees(np.arctan2(psis[:,1], psis[:,0]))
	return curvature[buff:-buff], r[buff:-buff], psis[buff:-buff]


def starify(*args):
	return (''.join((('%.6f'%i).rjust(14))  if not isinstance(i,int) else ('%d'%i).rjust(14) for i in args) + ' \n')[1:]
