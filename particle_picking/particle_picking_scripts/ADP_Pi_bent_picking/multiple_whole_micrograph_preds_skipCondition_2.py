...
def run_pick_on_micrograph(file_name):
	...
	for j in range(0, len(filaments)):
		...
		curvature, r, psis = spline_curv_estimate(filaments[j], len(filaments[j]), 13, 3, 40) # sampling, buff, min_threshold #10, 3, 50
		if(curvature != [] and len(curvature) >= 3):
			if(len(psis[np.argwhere(np.logical_and(100<curvature, curvature<1200))]) >= 3):
				#r_curv_range = r[np.argwhere(np.logical_and(10<curvature, curvature<1200))][:,0,:]
				r_curv_range = r[np.argwhere(np.logical_and(100<curvature, curvature<1200))][:,0,:] # corresponds to 0.004 nm^-1 curvature
				kept_curvs = curvature[np.argwhere(np.logical_and(100<curvature, curvature<1200))]
				plt.scatter(r_curv_range[:,1]-25, r_curv_range[:,0], c='red',s=5, alpha=0.5)
				rs.append(np.concatenate((r_curv_range, kept_curvs), axis=-1)); 
				psi_priors.append(psis[np.argwhere(np.logical_and(100<curvature, curvature<1200))])
